...
- M4AR exact-SHA Linux build/tests passed, but live human observation saw the fresh window close twice before MTProto UI could be verified. The prior launch mechanism may have tied the GUI lifetime to the Executor shell/session, so a real client crash is not yet proven.
- Current authorized phase: M4ARR detached client lifetime/crash diagnosis from exact release SHA. No code fix is authorized yet; first sync remains deferred.
