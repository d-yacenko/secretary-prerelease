"""Closed, zero-provider protocol for the M4AW2 human-shell eligibility probe."""
...
RELEASE = "2db36510fe884eadc40d63fed8661ed3627f1cbb"
TERMINAL = "M4AW2_TERMINAL"
FAILURE_STAGES = {
    "STAGE_0_REPO",
    "STAGE_0_RELEASE_REF",
...
def parse_output(text: str) -> str:
    ...
    for key in BOOLEANS:
        if reader.pos < len(reader.lines) and reader.lines[reader.pos].startswith(
            ...
            return result
        reader.boolean(key)
    counts = {key: reader.count(key) for key in COUNTS}
    imported = counts["IMPORTED_OBJECT_COUNT"]
    active = counts["TRANSPORT_VISIBLE_COUNT"]
    if not (
        active <= imported
        and counts["INBOUND_COUNT"] + counts["OUTBOUND_COUNT"] == imported
        and counts["INBOX_ELIGIBLE_COUNT"] <= active
        and counts["FIRST_PAGE_TOTAL_COUNT"] <= 50
        and counts["FIRST_PAGE_TELEGRAM_COUNT"] <= counts["FIRST_PAGE_TOTAL_COUNT"]
        and counts["FIRST_50_TELEGRAM_COUNT"] <= 50
    ):
        raise ValueError()
    reader.take("TELEGRAM_NETWORK_CALLS", ("0",))
...
try:
    with SessionLocal(autoflush=False) as db:
        ...
        inbound = int(db.scalar(select(func.count()).select_from(Object).where(
            *exact, Object.metadata_["direction"].as_string().is_distinct_from("outbound"))) or 0)
        outbound = int(db.scalar(select(func.count()).select_from(Object).where(
            *exact, Object.metadata_["direction"].as_string() == "outbound")) or 0)
        recent = RecentSourceService(db, account.user_id)
        inbox_eligible = int(db.scalar(select(func.count()).select_from(Object).where(
            *exact, recent._eligible_filters())) or 0)
        first_page = recent.list_page(limit=30).items
        first_50 = recent.list_page(limit=50).items
        def exact_item(item):
            metadata = item.metadata_ or {}
            return (
                item.provider == "telegram"
                and item.kind == "chat_message"
                and metadata.get("transport") == "mtproto"
                and metadata.get("account_id") == str(account.id)
                and metadata.get("peer_id") == str(selection.peer_id)
            )
        emit("IMPORTED_OBJECT_COUNT", imported)
        emit("TRANSPORT_VISIBLE_COUNT", active)
        emit("INBOUND_COUNT", inbound)
        emit("OUTBOUND_COUNT", outbound)
        emit("INBOX_ELIGIBLE_COUNT", inbox_eligible)
        emit("FIRST_PAGE_TOTAL_COUNT", len(first_page))
        emit("FIRST_PAGE_TELEGRAM_COUNT", sum(exact_item(item) for item in first_page))
        emit("FIRST_50_TELEGRAM_COUNT", sum(exact_item(item) for item in first_50))
        emit("TELEGRAM_NETWORK_CALLS", 0)
        emit("M4AW2_TERMINAL", "success")
...
def main() -> int:
    ...
    if sys.argv[1] == "validate":
        try:
            ...
            print("MANUAL_M4AW2_BLOCKED=remote_incomplete")
            return 2
        except (ValueError, OSError):
            print("MANUAL_M4AW2_BLOCKED=remote_protocol")
            return 2
        print(text, end="")
