#!/usr/bin/env python3
"""Stream the rehearsal helper into an isolated production one-shot container.

The production Git checkout stays on the deployed release. This wrapper does
not deploy, recreate, or restart the long-running API/worker. Host-side probes
read only the Telegram AI flag, then a temporary `docker compose run --rm
--no-deps` process receives the helper and process-local AI=true.
"""
...
HELPER_FILE = HERE / "telegram_production_rehearsal.py"
...
def oneshot_compose_command(run_id: str, helper_path: str) -> list[str]:
    ...
    return [
        ...
        "-e",
        "REHEARSAL_LIVE_CONFIRM=reviewed",
        "-e",
...
def build_remote_program(run_id: str, helper_source: str) -> str:
    if RUN_ID_RE.fullmatch(run_id) is None:
        raise RemoteBlocked("run_id")
...
def git(*args):
    ...
    if proc.returncode != 0:
        sys.stdout.write("REHEARSAL_REMOTE_BLOCKED=git\\n")
        raise SystemExit(2)
...
def service_flag(service):
    ...
    if proc.returncode not in (0, 1):
        sys.stdout.write("REHEARSAL_REMOTE_BLOCKED=long_running_probe\\n")
        raise SystemExit(2)
    return proc.stdout.strip()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        program = build_remote_program(args.run_id, HELPER_FILE.read_text(encoding="utf-8"))
        ...
        sys.stdout.write(f"REHEARSAL_REMOTE_BLOCKED={exc}\n")
        return 2
