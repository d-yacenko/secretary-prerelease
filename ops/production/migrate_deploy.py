#!/usr/bin/env python3
"""Fail-closed entrypoint for the one authorized schema-bearing rollout."""
...
REMOTE_HELPER = Path(__file__).resolve().with_name("remote_migrate_deploy.py")
AUTHORIZED_ROLLBACK = "5cce4b57b14e0052a038acae1354a2821a2bb77b"
FROM_ALEMBIC = "0041"
TO_ALEMBIC = "0046"
...
def _require_exact_migration_delta(rollback: str, release: str) -> None:
    rows = _git("diff", "--name-status", "--find-renames", rollback, release, "--", "backend/alembic")
    changed: set[str] = set()
    ...
    if changed != AUTHORIZED_MIGRATIONS:
        raise DeployError("migration delta is not exactly 0042 through 0046")
    ...
    for path in infra:
        if _git("diff", "--quiet", rollback, release, "--", path) != "":
...
def main() -> int:
    ...
    try:
        ...
        if args.from_alembic != FROM_ALEMBIC or args.to_alembic != TO_ALEMBIC:
            raise DeployError("only the authorized 0041 to 0046 transition is supported")
        ...
        known_hosts = _verified_known_hosts(target["ssh_target"], target["ssh_port"], target["host_key_sha256"])
        with tempfile.NamedTemporaryFile("w", encoding="utf-8") as hosts:
            hosts.write(known_hosts)
            hosts.flush()
            values = [
                "python3", "-", "--release-sha", release, "--rollback-sha", rollback,
                "--from-alembic", FROM_ALEMBIC, "--to-alembic", TO_ALEMBIC,
                "--origin-url", CANONICAL_ORIGIN, "--health-url", target["health_url"],
            ]
            command = f"cd {shlex.quote(target['repository_path'])} && " + " ".join(shlex.quote(v) for v in values)
            proc = subprocess.run([
                "ssh", "-p", str(target["ssh_port"]), "-o", "BatchMode=yes",
                "-o", "StrictHostKeyChecking=yes", "-o", f"UserKnownHostsFile={hosts.name}",
        ...
        print("MIGRATION_DEPLOYMENT_BLOCKED=preflight", file=sys.stderr)
        return 2
