...
- Bot API retirement is now selected after production acceptance of the ordinary MTProto transport. Cross-transport deduplication is intentionally not being built. Retirement is staged: first disable Bot ingress/link/send behavior in application code while preserving MTProto and historical Bot-derived objects; then, under separate production authorization, delete/disable the Telegram webhook and remove Bot credentials/config; only later consider removal of dead Bot-only code/UI/tests and any destructive legacy-schema cleanup.
