...
6. Production SSH must use the canonical target/pinned-host-key contract. For explicitly authorized BREAK-GLASS read-only diagnostics, SSH readiness means a real pinned **public-key-only** no-op succeeds (`PasswordAuthentication=no`, `KbdInteractiveAuthentication=no`, `PreferredAuthentications=publickey`). `BatchMode=yes`, `ssh-add -l`, `SSH_AUTH_SOCK`, `ssh -G`, identity-file counts, and agent-key enumeration are not credential-availability gates. Normal deployment still uses the committed deploy harness unchanged. Never create a new production credential, copy a private key into the repository, or ask the user to paste key/password/passphrase/token material.
7. If canonical Git/bootstrap or the actual pinned public-key-only no-op is unavailable, emit one concise sanitized blocker and STOP. If only `BatchMode=yes` disagrees with a proven working public-key sandbox SSH path, continue the authorized read-only task without treating that as missing credentials.
